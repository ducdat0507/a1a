...
function init() {
    ...
    scene.append(controls.base({
        ...
        onclick() {
            ...
            startAnimation((t) => {
                ...
                machine.position.fy = ease.quart.inout(t) * 100;
    ...
    scene._base.append(controls.rect({
        position: { fx: 20, fy: 0, sx: 0, sy: 1 },
        size: { fx: -40, fy: -120, sx: 1, sy: 1 },
        radius: 30,
        fill: "#1f1f1f",
    }), "machine");
    ...
    scene._base._machine._button.append(controls.text({
        position: { fx: 0, fy: -95, sx: 0.5, sy: 0.5 },
        size: 16,
        text: "A =",
        fill: "#8f8f8f",
    }), "label");
    ...
    scene._base._machine._button.append(controls.rect({
        position: { fx: 20, fy: -80, sx: 0, sy: 0.5 },
        size: { fx: -40, fy: 150, sx: 1, sy: 0 },
        radius: 10,
        fill: "#000000",
    }), "valueBackground");
...
function doPointerEvent(pos, cts, event) {
    for (let ct of cts) {
        if (pos.x >= ct.rect.x && pos.y >= ct.rect.y
            && pos.x <= ct.rect.x + ct.rect.width
            && pos.y <= ct.rect.y + ct.rect.height) {
            ct[event]();
            if (ct.controls.length) doPointerEvent(pos, ct.controls, event);
        }
    }
}